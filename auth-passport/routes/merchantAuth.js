...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, new Date().getTime().toString() + '.' + MIME_TYPE_MAP[file.mimetype]);
    }
...
//MERCHANT REGISTER
router.post("/register",multer({ storage: storage }).single("photo"), async (req, res) => {
...
router.patch("/profile",jwtHelper.verifyJwtToken ,async (req, res) => {
   ...
   const updated = await merchant.updateOne({user:req._id}, { $set: { availability: availability} });
